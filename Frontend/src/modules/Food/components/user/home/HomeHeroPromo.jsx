...
export default function HomeHeroPromo({
  ...
  const hasBanners = Array.isArray(activeBanners) && activeBanners.length > 0;
  const currentBanner = hasBanners
    ? activeBanners[currentBannerIndex % activeBanners.length]
    : null;
  const bannerImage = currentBanner?.imageUrl || currentBanner?.image || null;
  ...
  return (
    ...
    >
      {/* 1. Dynamic Promotional Banner (Rendered ONLY if active banners exist) */}
        ...
        <div className="w-full lg:col-span-8">
          ...
          >
            ...
            {activeBanners.length > 1 && (
              <div className="absolute bottom-2.5 left-1/2 -translate-x-1/2 z-20 flex items-center gap-1.5 px-2.5 py-1 bg-black/40 backdrop-blur-md rounded-full border border-white/15">
      ...
      <div
        className={`grid grid-cols-2 gap-2.5 sm:gap-3.5 ${
          hasBanners && bannerImage
            ? "lg:grid-cols-1 lg:col-span-4 lg:h-[250px]"
            : "lg:grid-cols-2 lg:col-span-12 lg:h-[135px]"
        }`}
      >
        {/* Card 1: Tiffin Services */}
        <Link
          to="/food/user/tiffin"
          className="group relative flex flex-col justify-between h-[118px] sm:h-[135px] lg:h-full rounded-[20px] overflow-hidden p-3 sm:p-3.5 bg-gray-950 border border-white/15 shadow-[0_6px_20px_rgba(0,0,0,0.25)] hover:shadow-xl transition-all duration-300 active:scale-[0.98]"
        >
          <img
            src={tiffinCardBg}
            alt="Tiffin Services"
